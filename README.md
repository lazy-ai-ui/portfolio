...
js/
  ...
  wheel.js                — флоу по дуге: липкая сцена, шаг от позиции прокрутки; режим двух ролей (Nearby)
...
Vercel, подключён к этому репозиторию. Пуш в `main` → пересобирается сам,
руками ничего заливать не нужно. Сборки нет: публикуется корень репозитория
как есть, build command пустой.
