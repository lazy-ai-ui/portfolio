...
(function () {
  ...
  function init(wh) {
    ...
    /* Куда выгибается дуга. Правило одно: выпуклость всегда смотрит в сторону
       текста — концы дуги у внешнего края страницы, середина тянется к
       подписи, которую в этот момент читают.

       Поэтому у колесика с макетом слева (по умолчанию) выпуклость вправо, у
       .wh--right — зеркально влево. У двух дуг возврата текст между ними,
       значит обе выгибаются внутрь: левая вправо, правая влево.

       +1 — выпуклость вправо, -1 — влево. Множитель уходит в place(). */
    var duo = wh.classList.contains('wh--duo');
    var right = wh.classList.contains('wh--right');
    ...
    for (var g = 0; g < arcs.length; g++) {
      ...
      if (list.length) {
        groups.push({
          arc: arcs[g],
          list: list,
          bulge: duo ? (groups.length === 0 ? 1 : -1) : (right ? -1 : 1)
        });
      }
    }
    ...
    function render(i) {
      ...
      for (var g = 0; g < groups.length; g++) {
        ...
        for (k = 0; k < list.length; k++) {
          ...
          place(list[k].el, k - a, groups[g].bulge);
        }
