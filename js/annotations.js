...
(function(){
  ...
  /* Россыпь сходится к цвету карточки, а карточка меняется вместе с темой,
     поэтому обе точки маршрута — старт и финиш — берутся из палитры. */
  var SURF=Theme.rgb('--surface-rgb','255,255,255');
  var DUST=Theme.rgb('--dust-rgb','214,211,207');
  var ACC=Theme.rgb('--accent-rgb','228,87,58');
  Theme.onChange(function(){
    SURF=Theme.rgb('--surface-rgb','255,255,255');
    DUST=Theme.rgb('--dust-rgb','214,211,207');
    ACC=Theme.rgb('--accent-rgb','228,87,58');
  });
...
function init(anno,DATA){
  ...
  Fx.prototype.draw=function(gOf){
    ...
    for(var i=0;i<P.length;i++){
      ...
      /* цвет ведём от тёплой серой россыпи к фону карточки,
         коралловые квадраты приходят к нему только на финише */
      var cr,cg,cb;
      if(q.accent){
        var m=Math.max(0,(g-0.5)*2);
        cr=ACC[0]+(SURF[0]-ACC[0])*m;
        cg=ACC[1]+(SURF[1]-ACC[1])*m;
        cb=ACC[2]+(SURF[2]-ACC[2])*m;
      } else {
        cr=DUST[0]+(SURF[0]-DUST[0])*g;
        cg=DUST[1]+(SURF[1]-DUST[1])*g;
        cb=DUST[2]+(SURF[2]-DUST[2])*g;
      }
